...
exports.plugin = {
    ...
    async register(server, pluginOptions) {
        ...
        const connect = async function (connectionOptions) {
            ...
            const client = new MongoClient(connectionOptions.url, connectionOptions.settings);
            await client.connect();
            const db = await client.db(connectionOptions.db);
        ...
        try {
            ...
            expose.client = options.lengh === 1 ? results[0].client : results.map(r => r.client);
        }
        catch (err) {
