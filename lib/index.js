...
exports.plugin = {
  async register(server, pluginOptions) {
    const options = await optionsSchema.validate(pluginOptions).value;
    ...
    const connect = async function (connectionOptions) {
      if (
        ...
        typeof connectionOptions.settings.promiseLibrary === "string"
      ) {
        connectionOptions.settings.promiseLibrary = require(connectionOptions
          .settings.promiseLibrary);
      }
      ...
      server.log(
        ["hapi-mongodb", "info"],
        "MongoClient connection created for " +
    ...
    try {
      ...
      expose.db =
        options.length === 1 ? results[0].db : results.map((r) => r.db);
      expose.client =
        options.length === 1 ? results[0].client : results.map((r) => r.client);
    } catch (err) {
